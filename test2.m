...
for s = 1:nSteps
    ...
    n_here      = interp2(n_all_s, jj, ii, 'linear', n_background);
    ...
    if mod(s, sample_every)==0
        ...
        contour(x, y, double(obj2), [0.5 0.5], 'c', 'LineWidth', 1.0);    % PDMS edges
        contour(x, y, double(obj1), [0.5 0.5], 'y', 'LineWidth', 1.0);    % sensor edges

        % plot rays (intensity -> alpha)
        for rr = 1:numel(traj_x)
            if numel(traj_x{rr}) > 1
                plot(traj_x{rr}, traj_y{rr}, '-', 'LineWidth', 1.0, ...
                    'Color', [1 0 0 max(0.05, min(1,I(rr)))]);
            end
        end
        drawnow;
        if movvar==1
...
hit = abs(xr - x_img) < ds;                % crude gating near plane
y_hits = yr(hit); I_hits = I(hit);
if ~isempty(y_hits)
